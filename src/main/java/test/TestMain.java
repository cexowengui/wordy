...
import core.util.HttpClientUtils;
...
public class TestMain {
	...
	public void Test() throws IOException {
		...
		try {
			//下面每个测试用例都是独立的，之间没有任何关系，每个测试用例都是清空数据库重新开始构建测试数据的
			 //this.TestRegistry();
			 this.TestLogin();
			// this.TestAddFriend();
			// this.TestTalk();
	...
	public void TestRegistry() throws Exception {
		...
		requestParams.put("userNum", String.valueOf(userNum));
		String response = HttpClientUtils.httpGet("http://127.0.0.1:8080/message/user", requestParams);
		User user = JsonUtil.JsonToObject(response, User.class);
		if (user.getUserName().equals("xww") && user.getUserPasswd().equals("passwd") && user.getUserNum() == userNum) {
	...
	public void TestLogin() throws Exception {
		...
		HashMap<String, String> requestParams = new HashMap<>();
		requestParams.put("userNum", String.valueOf(userNum));
		String response = HttpClientUtils.httpGet("http://127.0.0.1:8080/message/socket", requestParams);
	...
	public void TestAddFriend() throws IOException, SQLException, InterruptedException {
		...
		// TestCase.userLogin(mySocket, userNum1,
		// "passwd");跟服务器打交道的请求不需要登录，只有两个客户端之间聊天需要登录
		TestCase.addFriend(mySocket, userNum1, userNum2);// xww1添加xww2作为好友
		TestCase.addFriend(mySocket, userNum1, userNum3);// xww1添加xww3作为好友
		// 调用服务端的http接口请求这三个人的详细信息，看user_friends里面是不是有正确信息
		// todo
		TestCase.closeResource(mySocket);
	}
